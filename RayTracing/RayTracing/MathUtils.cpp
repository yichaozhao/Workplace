...
Geom::GeometryExtent3D CalculateGeometryExtent(int argCount, Geom::Point3D* ...)
{
	...
	{
		Geom::Point3D* pPoint = va_arg(pointList, Geom::Point3D*);
		assert(pPoint);

		min_x = std::fmin(min_x, pPoint->GetX());
		min_y = std::fmin(min_y, pPoint->GetY());
		min_z = std::fmin(min_z, pPoint->GetZ());

		max_x = std::fmax(max_x, pPoint->GetX());
		max_y = std::fmax(max_y, pPoint->GetY());
		max_z = std::fmax(max_z, pPoint->GetZ());
	}
